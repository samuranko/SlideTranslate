    ...
    if (slide.texts.length > 0) {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
            ...
            // FIX: Add responseSchema for robust JSON output.
            responseSchema: {
              type: Type.ARRAY,
...
export const translateDocx = async (
  ...
  const translatedDocData: DocxData = { paragraphs: [] };
  const originalTexts = docData.paragraphs.map(p => p.original);
  
  onProgress(10);
  const translatedTexts = await translateTextChunks(originalTexts, targetLanguage, glossary, 'document');
  onProgress(90);

  translatedDocData.paragraphs = originalTexts.map((original, i) => ({
    original,
    translated: translatedTexts[i] || original,
  }));
